  ...
  input.addEventListener('blur', function (e) {
    if (input.value === '') {
      labels.forEach(function (lab) {
        input.getAttribute('id') === lab.getAttribute('for') &&
        lab.classList.contains('moved')
          ? lab.classList.remove('moved')
          : null;
        checkings.forEach(function (checkBX) {
          checkBX.classList.contains(input.getAttribute('id')) &&
          checkBX.childElementCount === 1
            ? remove(checkBX)
            : null;
        });
      });
...
function showResult(result) {
  result
    ? (message.textContent = 'Correct writing!')
    : (message.textContent = 'Wrong writing. Please try again.');
...
function content(level) {
  if (level === 'strong') {
    ...
    strOfPass.textContent =
      'Your password is ' + level + '! Check the rules for improving it.';
